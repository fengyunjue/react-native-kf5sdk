...
import com.kf5.sdk.helpcenter.mvp.presenter.HelpCenterTypePresenter;
...
public class HelpCenterTypeActivity extends BaseActivity<HelpCenterTypePresenter, IHelpCenterTypeView> implements
        IHelpCenterTypeView, View.OnClickListener, View.OnKeyListener,
    ...
    protected void initWidgets() {
        ...
        UIUtils.setNavBarUI(topLayout, mBackImage, mTitleView, mRightView);
    }
    ...
    @Override
    public Loader<HelpCenterTypePresenter> onCreateLoader(int id, Bundle args) {
        return new PresenterLoader<>(this, new PresenterFactory<HelpCenterTypePresenter>() {
            @Override
            public HelpCenterTypePresenter create() {
                return new HelpCenterTypePresenter(HelpUseCaseManager.provideHelpCenterTypeCase());
            }
        });
    }

    @Override
    public void onLoadFinished(Loader<HelpCenterTypePresenter> loader, HelpCenterTypePresenter data) {
        super.onLoadFinished(loader, data);
        showDialog = true;
        presenter.getForumListById(HelpCenterRequestType.DEFAULT);
    }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (lastItem == listItem.size() && scrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE) {
            if (nextPage != -100 && nextPage != 1) {
                ...
                else presenter.getForumListById(HelpCenterRequestType.DEFAULT);
            } else {
